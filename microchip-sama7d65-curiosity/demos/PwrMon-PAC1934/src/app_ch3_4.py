...
# PAC1934 Power Monitor Demo - Channels 3 & 4 (VDDCORE + VDD_CPU)
# Monitors the VDDCORE and VDD_CPU power rails and publishes telemetry to /IOTCONNECT.
...
CHANNELS = [3, 4]
CHANNEL_LABELS = {3: "VDDCORE", 4: "VDD_CPU"}
...
try:
    ...
    print("Monitoring Channels 3 & 4: %s" % ", ".join(CHANNEL_LABELS[ch] for ch in CHANNELS))
    ...
    while True:
        ...
        for ch in CHANNELS:
            data = pac.get_channel(ch)
            label = CHANNEL_LABELS[ch].lower()
            telemetry[f'{label}_vbus_mv'] = data['vbus_mv']
            telemetry[f'{label}_isense_ma'] = data['isense_ma']
            telemetry[f'{label}_power_mw'] = data['power_mw']
            print("  %s: %.1f mV, %.2f mA, %.2f mW" % (
                CHANNEL_LABELS[ch], data['vbus_mv'], data['isense_ma'], data['power_mw']
            ))
