...
import numpy as np
from avnet.iotconnect.sdk.lite import Callbacks, Client, DeviceConfig, DeviceConfigError, C2dCommand
...
TELEMETRY_SECS = max(1.0, float(os.getenv("KWS_TELEMETRY_SECS", "2")))
last_error = ""
...
def on_command(msg: C2dCommand):
    ...
    if msg.command_name == "get-status":
        ...
        message = f"listening={status['listening']} label={status['kws_label']} confidence={status['kws_confidence']}"
        if msg.ack_id is not None:
        ...
        while True:
            ...
            if LISTENING_ENABLED:
                try:
                    ...
                    time.sleep(2)
                send_telemetry()
                next_idle_telemetry = time.time() + TELEMETRY_SECS
                continue

            if time.time() >= next_idle_telemetry:
                send_telemetry()
                next_idle_telemetry = time.time() + TELEMETRY_SECS
            time.sleep(0.25)
