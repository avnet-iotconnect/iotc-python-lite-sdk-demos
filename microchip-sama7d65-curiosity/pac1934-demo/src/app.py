...
# PAC1934 Power Monitor Demo - All 4 Channels
# Reads VDD_3V3, VDD_DDR_IO, VDDCORE, and VDD_CPU power rails
# and publishes telemetry to /IOTCONNECT.
...
from py_pac193x import PAC193x, CHANNEL_LABELS
...
def build_telemetry(channels):
    """Build telemetry dict from all 4 PAC1934 channels."""
    telemetry = {
    ...
    for ch in range(1, 5):
        data = channels[ch]
        prefix = data['label'].lower()
        telemetry[f'{prefix}_vbus_mv'] = data['vbus_mv']
        telemetry[f'{prefix}_isense_ma'] = data['isense_ma']
        telemetry[f'{prefix}_power_mw'] = data['power_mw']
    return telemetry
...
try:
    ...
    print("Monitoring all 4 channels: %s" % ", ".join(CHANNEL_LABELS.values()))
    ...
    while True:
        ...
        channels = pac.get_all_channels()
        telemetry = build_telemetry(channels)

        # Print summary to console
        for ch in range(1, 5):
            d = channels[ch]
            print("  %s: %.1f mV, %.2f mA, %.2f mW" % (
                d['label'], d['vbus_mv'], d['isense_ma'], d['power_mw']
            ))
