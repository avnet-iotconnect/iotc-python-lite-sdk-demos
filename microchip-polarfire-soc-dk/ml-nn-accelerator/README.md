...
The Tiny-NN Accelerator uses a fixed-point Tiny-NN style classifier (`int8` weights with `int16/int32` accumulation). Each inference starts from `256` time-domain samples, extracts `32` features, builds a `12`-element hidden representation (`6` positive + `6` negative channels), then produces scores across `6` classes.
...
The Tiny-NN Accelerator often shows modest HW gains when workload size and batch are large enough to offset acceleration overhead.

## 2. Program FPGA with Tiny-NN Accelerator Image

The quickstart programmed the board with the stock Microchip reference design. This step replaces it with a demo-specific FPGA image that includes the Tiny-NN accelerator in the FPGA fabric, which is required for `hw` mode inference.

1. Open FlashPro Express.
2. Download the Tiny-NN Accelerator FPGA job file [here](https://raw.githubusercontent.com/avnet-iotconnect/iotc-python-lite-sdk-demos/main/microchip-polarfire-soc-dk/ml-nn-accelerator/assets/fpga-job/MPFS_DISCOVERY_KIT.job) (right-click, "save as").
...
2. Download and import the device template [here](https://raw.githubusercontent.com/avnet-iotconnect/iotc-python-lite-sdk-demos/main/microchip-polarfire-soc-dk/ml-nn-accelerator/microchip-polarfire-tinyml-template.json). (right-click and "save link as")
3. Save.
...
2. Download dashboard template [here](https://raw.githubusercontent.com/avnet-iotconnect/iotc-python-lite-sdk-demos/main/microchip-polarfire-soc-dk/ml-nn-accelerator/mchp-nn-dashboard-template.json). (right-click and "save link as"), then click **Import Dashboard** and upload the JSON file.
...
```bash
wget https://raw.githubusercontent.com/avnet-iotconnect/iotc-python-lite-sdk-demos/main/microchip-polarfire-soc-dk/ml-nn-accelerator/package.tar.gz
```
...
<img src="../images/mchp-polarfire-nn-dashboard.jpg" alt="Tiny-NN Accelerator dashboard result" width="600" />
...
- `developer-guide.md`: full SmartHLS + Libero rebuild flow
