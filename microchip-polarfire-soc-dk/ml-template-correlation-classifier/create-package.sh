...
SW_ELF="${SRC_DIR}/runtimes/invert_and_threshold.no_accel.elf"
HW_ELF="${SRC_DIR}/runtimes/invert_and_threshold.accel.elf"
...
if [[ ! -f "${SW_ELF}" || ! -f "${HW_ELF}" ]]; then
  ...
  wget -q --show-progress -O "${SW_ELF}" "${S3_BASE}/invert_and_threshold.no_accel.elf"
  wget -q --show-progress -O "${HW_ELF}" "${S3_BASE}/invert_and_threshold.accel.elf"
fi
