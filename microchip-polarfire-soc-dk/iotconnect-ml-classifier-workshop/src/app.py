...
def parse_classify_args(args):
    ...
    if len(args) == 1 and args[0].strip().startswith("{"):
        ...
        input_class = int(payload.get("class", payload.get("input_class", input_class)))
    ...
    if batch < 1 or batch > 10000:
        raise ValueError("batch must be in range 1..10000")
...
def _run_batch_inference(mode: str, input_class: int, seed: int, batch: int):
    if batch == 1:
        result = ml_runner.run_inference(mode=mode, input_class=input_class, seed=seed)
        return {
            "requested_mode": mode,
            ...
            "pred": result["pred"],
            "pred_mode_count": 1,
            "match_count": 1 if result["pred"] == input_class else 0,
            "match_rate": 1.0 if result["pred"] == input_class else 0.0,
            "total_time_s": result["time_s"],
            "avg_time_s": result["time_s"],
    ...
    for i in range(batch):
        run_seed = seed + i
        result = ml_runner.run_inference(mode=mode, input_class=input_class, seed=run_seed)
        exec_mode = result["exec_mode"]
        total_time_s += result["time_s"]
        pred_hist[result["pred"]] = pred_hist.get(result["pred"], 0) + 1
        if result["pred"] == input_class:
            matched += 1
        scores = result.get("scores", [])
...
def on_command(msg: C2dCommand):
    ...
    if msg.command_name == COMMAND_STATUS:
        try:
            include_leds = True
            if len(msg.command_args) == 1 and msg.command_args[0].strip().startswith("{"):
                payload = json.loads(msg.command_args[0])
                include_leds = bool(payload.get("include_leds", True))
            else:
                for raw in msg.command_args:
                    token = raw.strip().lower()
                    if token in ("leds", "include_leds", "full"):
                        include_leds = True
                    elif token in ("noleds", "basic"):
                        include_leds = False
    ...
    if msg.command_name == COMMAND_FILE_DOWNLOAD:
        if len(msg.command_args) != 1:
            ...
            return
        try:
            response = requests.get(msg.command_args[0], timeout=30)
