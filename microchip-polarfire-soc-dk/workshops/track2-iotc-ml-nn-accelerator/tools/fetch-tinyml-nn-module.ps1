...
try {
  ...
  if ($LocalModulePath -and $LocalModulePath.Trim().Length -gt 0) {
    ...
    $moduleSource = Join-Path $repoRoot.FullName "microchip-polarfire-soc-dk\workshops\track2-iotc-ml-nn-accelerator\assets\smarthls-module\tinyml_nn"
    if (-not (Test-Path $moduleSource)) {
      $legacyModuleSource = Join-Path $repoRoot.FullName "microchip-polarfire-soc-dk\track2-iotc-ml-nn-accelerator\assets\smarthls-module\tinyml_nn"
      if (Test-Path $legacyModuleSource) {
        $moduleSource = $legacyModuleSource
      } else {
        throw "tinyml_nn module not found in downloaded repo. Checked: $moduleSource and $legacyModuleSource. Use a branch/tag that contains microchip-polarfire-soc-dk/workshops/track2-iotc-ml-nn-accelerator/assets/smarthls-module/tinyml_nn, or pass -LocalModulePath."
      }
    }
  }
