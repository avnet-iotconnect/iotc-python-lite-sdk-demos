...
NO_ACCEL = "tinyml_nn.no_accel.elf"
ACCEL = "tinyml_nn.accel.elf"

RESULT_RE = re.compile(
    r"Tiny-(?:ML|NN) demo \((?P<exec_mode>sw|hw)\): input_class=(?P<input_class>\d+) pred=(?P<pred>\d+) time=(?P<time_s>[0-9.]+) s"
)
...
def _parse_output(output: str) -> dict:
    ...
    if not result_match:
        raise RuntimeError("Unable to parse Tiny-NN result line from process output.")
