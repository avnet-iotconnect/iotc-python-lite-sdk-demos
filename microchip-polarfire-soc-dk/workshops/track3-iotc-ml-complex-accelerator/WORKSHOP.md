...
Track 3 is intentionally scaled to make HW acceleration easier to observe than Track 1/2 by combining a larger model with larger batch execution.
...
Track 3 uses a fixed-point multi-layer classifier (`int8` weights with `int16/int32` accumulation) so the same application logic can run in software or hardware with deterministic behavior. Each inference starts from `256` time-domain samples, extracts `64` features, then runs through two hidden layers (`96`, then `48`) before producing scores for `6` classes.

Model size is intentionally larger than earlier tracks to make the acceleration effect easier to see at the system level. The parameter set includes `W1[96x64]`, `W2[48x96]`, `W3[6x48]` (`11,040` weights) and `B1[96]`, `B2[48]`, `B3[6]` (`150` biases), for an approximate model-parameter footprint of `11.1 KiB` (`11,340` bytes).

The runtime is also designed for batch experiments. It supports up to `1024` inferences per request and allocates DMA-safe buffers at max capacity (`256 x 1024` inputs, `6 x 1024` outputs). This keeps run-to-run behavior stable while you scale batch size and compare SW and HW timing.

### 1.2 Built-In Application Flows and Their Purpose

Track 3 includes two complementary runtime flows: `classify` and `bench`.

The `classify` flow is the functional demonstration of the classifier engine. You select `sw` or `hw`, then classify one of six waveform classes (`0..5`, or `random`). The runtime generates a deterministic waveform with seed-controlled noise injection, runs inference, and publishes prediction-focused telemetry (`pred`, `scores_csv`, timing, and batch stats). This shows that the classification pipeline is working in both software and hardware modes on the same waveform families with noise present.

The `bench` flow is the performance demonstration. It runs SW, HW, or both on the same controlled input settings and batch size, then publishes benchmark telemetry (`ml_bench`) including `sw_avg_time_s`, `hw_avg_time_s`, `sw_total_time_s`, `hw_total_time_s`, `sw_match_rate`, and `hw_match_rate`. When both modes are run, it also computes `speedup_sw_over_hw`.

In practical workshop use, start with `classify` to confirm model behavior and prediction validity, then run `bench` to quantify acceleration and compare software/hardware consistency. In the end, you are validating both model performance (latency/throughput trend) and classification accuracy parity between hardware and software implementations.
...
2. Download FPGA project [link](https://raw.githubusercontent.com/avnet-iotconnect/iotc-python-lite-sdk-demos/main/microchip-polarfire-soc-dk/workshops/track3-iotc-ml-complex-accelerator/assets/fpga-job/MPFS_DISCOVERY_KIT.job) (right-click, "save as")
...
```bash
wget https://raw.githubusercontent.com/avnet-iotconnect/iotc-python-lite-sdk-demos/main/microchip-polarfire-soc-dk/workshops/track3-iotc-ml-complex-accelerator/package.tar.gz
```
...
2. Click **Import Dashboard** and upload the JSON file [../templates-iotconnect/mchp-track3-dashboard-template.json](https://raw.githubusercontent.com/avnet-iotconnect/iotc-python-lite-sdk-demos/main/microchip-polarfire-soc-dk/workshops/templates-iotconnect/mchp-track3-dashboard-template.json).
...
```
   ...
   - Expected result:
     - Separation between SW and HW average-time series is easier to observe.
...
load action=<start|stop|status|off> [workers=<n>] [duty=<pct>]
```

Valid values:

- `action`: `start`, `stop`, `status`, or `off` (`off` is an alias for `stop`)
