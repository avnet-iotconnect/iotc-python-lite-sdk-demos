...
# The QCS6490 has a Qualcomm msm_vidc hardware H264 encoder accessible via the
# standard V4L2 API (v4l2h264enc). The USB camera captures MJPG at 1280x720
# @ 30fps to stay within USB 2.0 bandwidth limits. jpegdec decodes the MJPG
# frames in software before v4l2h264enc re-encodes to H264 using the on-chip
# hardware codec. The hardware encoder requires NV12 input, so a videoconvert
# step is inserted between jpegdec and v4l2h264enc.
camera_options = {
    "video": {
        "width": 1280,
        "height": 720,
...
def detect_video_device() -> Optional[str]:
    # The QCS6490 exposes several non-USB video nodes: /dev/video0 and /dev/video1
    # are the Qualcomm camera request manager and sync nodes, and /dev/video32 and
    # /dev/video33 are the msm_vidc hardware codec nodes. USB cameras appear under
    # a USB bus path in sysfs. We check for "usb" in the resolved sysfs path to
    # reliably select the USB camera regardless of which port it is plugged into.
    try:
...
def start_video_stream(
    ...
    # The USB camera is captured in MJPG format to stay within USB 2.0 bandwidth
    # limits at 720p/30fps. jpegdec decodes the MJPG frames before the Qualcomm
    # msm_vidc hardware H264 encoder (v4l2h264enc) re-encodes to H264. The hardware
    # encoder requires NV12 input, so videoconvert converts from the decoded YUV.
    ...
    # Ensure the KVS SDK libraries and GStreamer plugin are findable regardless
    # of whether /etc/profile.d/kvs-gstreamer.sh has been sourced in this session.
    kvs_lib_path = "/opt/kvs-producer-sdk-cpp/build"
    gst_env = os.environ.copy()
    existing_ld = gst_env.get("LD_LIBRARY_PATH", "")
    gst_env["LD_LIBRARY_PATH"] = f"{kvs_lib_path}:{existing_ld}" if existing_ld else kvs_lib_path
    gst_env["GST_PLUGIN_PATH"] = kvs_lib_path
    ...
    try:
        ...
        if return_code is not None:
            ...
            print("   - kvssink plugin is not installed (run install.sh)")
            print("   - Video device is not accessible")
            print("   - v4l2h264enc hardware encoder is not available")
