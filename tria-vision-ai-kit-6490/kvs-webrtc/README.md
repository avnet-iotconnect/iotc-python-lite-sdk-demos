...
Upgrades the /IOTCONNECT Starter Demo on the Tria Vision AI-KIT 6490 to the AWS Kinesis Video Streams (KVS) WebRTC live video streaming demo.
...
This demo streams live video from a USB camera through the Tria Vision AI-KIT 6490 to a browser or other WebRTC viewer via AWS Kinesis Video Streams (KVS) WebRTC. The device acts as a WebRTC MASTER: it connects to the KVS signaling channel provided by /IOTCONNECT and streams video to any VIEWER that connects.
...
The QCS6490 Robotics Reference Distro ships with `cffi` and `numpy` pre-installed and PyPI provides native aarch64 wheels for all remaining dependencies (`aiortc`, `av`/PyAV, `websockets`, `boto3`). Installation is a simple `pip install` — no system package workarounds are required.
...
```bash
cd /opt/demo
wget -O package.tar.gz https://raw.githubusercontent.com/avnet-iotconnect/iotc-python-lite-sdk-demos/main/tria-vision-ai-kit-6490/kvs-webrtc/package.tar.gz
...
2. Edit files in `tria-vision-ai-kit-6490/kvs-webrtc/src/` as needed.

3. Rebuild the package:
   ```bash
   cd tria-vision-ai-kit-6490/kvs-webrtc
