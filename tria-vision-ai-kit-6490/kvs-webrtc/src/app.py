...
# The QCS6490 has several non-USB video nodes: /dev/video0 and /dev/video1 are
# the Qualcomm camera subsystem, and /dev/video32 and /dev/video33 are the msm_vidc
# hardware codec nodes. For WebRTC, the hardware encoder is not used — aiortc
# handles encoding in Python. GStreamer captures raw RGB frames at 640x480 from
# the USB camera and pipes them to stdout for the frame reader thread.
camera_options = {
    "video": {
        "width": 640,
        "height": 480,
    ...
    if kvs_client.is_streaming():
        ...
        try:
            ...
            if not channel_arn:
                ...
                print("Ensure the device is created with the 'plitekvs' template.")
                return
