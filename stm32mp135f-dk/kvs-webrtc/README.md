...
Upgrades the /IOTCONNECT Starter Demo on the STM32MP135F-DK to the AWS Kinesis Video Streams (KVS) WebRTC live video streaming demo.
...
This demo streams live video from a USB camera through the STM32MP135F-DK to a browser or other WebRTC viewer via AWS Kinesis Video Streams (KVS) WebRTC. The device acts as a WebRTC MASTER: it connects to the KVS signaling channel provided by /IOTCONNECT and streams video to any VIEWER that connects.
...
```bash
cd /opt/demo
wget https://raw.githubusercontent.com/avnet-iotconnect/iotc-python-lite-sdk-demos/main/stm32mp135f-dk/kvs-webrtc/package.tar.gz
...
2. Edit files in `stm32mp135f-dk/kvs-webrtc/src/` as needed.

3. Rebuild the package:
   ```bash
   cd stm32mp135f-dk/kvs-webrtc
