...
def detect_video_device() -> Optional[str]:
    # On the STM32MP257F-EV1 the onboard camera interfaces (dcmipp, vdec, venc)
    # also appear as /dev/videoN nodes. To reliably pick a USB camera regardless
    # of which port it is plugged into or what number it is assigned, we check the
    # sysfs path for each video device: USB-connected devices have "usb" in their
    # resolved sysfs path while onboard devices do not.
    try:
