...
After navigating into the ```/package-create-and-deploy/package-contents/``` directory, modify ```app.py``` as desired. 
...
Inside of the ```/package-create-and-deploy/package-contents/``` directory is a bare-bones version of the ```install.sh``` script. This script gets automatically run on the target device when the package is received and extracted. 
...
## 3. Create Package
Within the ```/package-create-and-deploy/scripts/``` directory, run this command:
```
bash ./generate-package.sh
```
You now have a file called ```install-package.tar.gz``` in the ```/package-create-and-deploy/package-contents/``` directory.
