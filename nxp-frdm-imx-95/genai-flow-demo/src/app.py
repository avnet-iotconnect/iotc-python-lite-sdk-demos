...
def deploy_model(entry):
    ...
    # Boards with an Ara240 stage into the connector's model directory; boards
    # without one can still take a GGUF for llama.cpp, so stage there instead of
    # failing on a connector path that does not exist.
    have_ara = read_ara_status()[0] != "absent"
    stage_dir = config["model_deploy_dir"] if have_ara else os.path.join(config["llama_dir"], "models")
    os.makedirs(stage_dir, exist_ok=True)
    # download to the models filesystem (disk), never /tmp (may be RAM-backed)
    dl = os.path.join(stage_dir,
    ...
    # A GGUF runs on any board through llama.cpp - take that path first so a
    ...
    if gguf is not None:
        ...
        final = os.path.join(models_dir, stem + ".gguf")
        set_model_deploy_status("loading", name, "installing %s for llama.cpp" % stem)
        ...
        save_config(config)
        chat_llm.stop()
        _models_cache["v"] = ""          # force the picker to pick it up
        with telemetry_lock:
            telemetry["llm_model"] = stem
            telemetry["llm_backend"] = "cpu"
            telemetry["llm_models"] = read_model_inventory()
        set_model_deploy_status("ready", name, "%s installed and selected (llama.cpp)" % stem)
        print("Model push: %s installed as a GGUF and selected" % stem)
        return stem
...
if len(sys.argv) >= 2 and sys.argv[1] in ("--test-agent", "--test-llm", "--test-vlm", "--test-rag", "--test-push"):
    ...
    try:
        if sys.argv[1] == "--test-vlm":
            ...
            answer = deploy_model({"url": sys.argv[2], "file_name": sys.argv[3],
