...
def detect_video_device() -> Optional[str]:
    # The NXP FRDM-IMX93 has no onboard camera interfaces, so all /dev/videoN
    # nodes should be USB devices. We still check sysfs for "usb" in the path
    # to be safe and to pick the correct device if multiple USB cameras are attached.
    try:
...
def start_video_stream(
    ...
    # The C920 USB camera is captured in MJPG format to stay within USB 2.0
    # bandwidth limits at 720p/30fps. jpegdec decodes the MJPG frames before
    # x264enc re-encodes to H264. There is no hardware H264 encoder accessible
    # via V4L2 on the i.MX93 so software x264enc is used throughout.
    ...
    # Ensure the KVS SDK libraries, bundled libx264, and GStreamer plugins are
    # findable regardless of whether /etc/profile.d/kvs-gstreamer.sh has been sourced.
    ...
    try:
        ...
        if return_code is not None:
            ...
            print("   - x264enc plugin is not installed (check libgstx264.so in /usr/lib/gstreamer-1.0/)")
        ...
        print(
            "Install with: apt-get install gstreamer1.0-tools gstreamer1.0-plugins-base gstreamer1.0-plugins-good")
        return None
    except Exception as e:
