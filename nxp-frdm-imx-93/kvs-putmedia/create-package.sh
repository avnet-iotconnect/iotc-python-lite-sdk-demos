...
if ! ls "$KVS_LIBS_DIR"/*.so* > /dev/null 2>&1; then
  ...
  echo "Run ~/kvs-build-imx93.sh first to build the KVS SDK libraries."
  exit 1
fi
...
# Copy pre-built KVS SDK libs (including libgstx264.so and libx264.so.164) into libs/ subdirectory
mkdir -p "$STAGING_DIR/libs"
cp "$KVS_LIBS_DIR"/*.so* "$STAGING_DIR/libs/"
echo "Bundled $(ls "$STAGING_DIR/libs/" | wc -l) library files into package."
