...
Upgrades the /IOTCONNECT Starter Demo on the NXP FRDM i.MX93 to the AWS Kinesis Video Streams (KVS) WebRTC live video streaming demo.

> [!IMPORTANT]
> Complete the [/IOTCONNECT quickstart guide for the NXP FRDM i.MX93](https://github.com/avnet-iotconnect/iotc-python-lite-sdk-demos/blob/main/nxp-frdm-imx-93/README.md) before proceeding.

## 1. Introduction

This demo streams live video from a USB camera through the NXP FRDM i.MX93 to a browser or other WebRTC viewer via AWS Kinesis Video Streams (KVS) WebRTC. The device acts as a WebRTC MASTER: it connects to the KVS signaling channel provided by /IOTCONNECT and streams video to any VIEWER that connects.

Unlike the KVS PutMedia demo, WebRTC is a peer-to-peer protocol — the video is delivered directly between the device and the viewer with very low latency. No pre-built C++ SDK libraries are needed; all WebRTC signaling and media encoding is handled in Python by `aiortc` and `boto3`.

The NXP i.MX93 Yocto image ships with `numpy` pre-installed and PyPI provides native aarch64 wheels for all remaining dependencies (`aiortc`, `av`/PyAV, `websockets`, `boto3`, `cffi`). Installation is a simple `pip install` — no system package workarounds are required.

## 2. Set Up Hardware and Template

1. Plug a USB camera into a USB-A port on the NXP FRDM i.MX93.

> [!TIP]
> Verify the camera is detected by running `ls /dev/video*` on the device. The USB camera typically appears as `/dev/video0` and `/dev/video1` (two nodes for the same device). The app automatically identifies the USB camera by inspecting the hardware path of each video device in sysfs.
...
```bash
cd /opt/demo
wget -O package.tar.gz https://raw.githubusercontent.com/avnet-iotconnect/iotc-python-lite-sdk-demos/main/nxp-frdm-imx-93/kvs-webrtc/package.tar.gz
tar -xzf package.tar.gz --overwrite
bash ./install.sh
```

> [!NOTE]
> The install downloads several Python packages (~50 MB total). Warning messages in the console are expected and can be ignored.
...
2. Edit files in `nxp-frdm-imx-93/kvs-webrtc/src/` as needed.

3. Rebuild the package:
   ```bash
   cd nxp-frdm-imx-93/kvs-webrtc
