...
Next, naviagte to the ```update-files``` directory:
```
cd ../update-files
```
You will need to make these slight modifications to the ```install.sh``` script for this update so only the required actions are executed:
...
From here, you have the option to push the update to your devices directly from you host machine's console as an OTA update (see step 6A), through an API device command (see step 6B), or you can upload the payload to the online IoTConnect platform and push an OTA update from there (see step 6C).
...
## 6B. Push Update as Command From Host Machine Console
...
## 6C. Upload/Push OTA Update in IoTConnect Online Platform
