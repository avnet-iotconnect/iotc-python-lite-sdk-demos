...
## 2. Arrange Files and Create Update Payload
Inside of the cloned repo, navigate to the ```scripts``` folder in this directory:
```
cd ./nxp-frdm-imx-93/dms-demo/scripts
```
Then run these commands to move the Driver Monitoring System AI Demo files into the correct folder for creating an update package, and then navigate to that folder:
```
bash ./move-files.sh
...
```
Inside of the ```update-files``` directory you should now see a file called ```update-payload.tar.gz```

## 3. Launch IoTConnect Program on Device
For your board to receive the update, it must be actively connected to IoTConnect. If it is not already, do this by running the main IoTConnect program on your board called ```app.py```:
...
From here, you have the option to push the update to your devices directly from you host machine's console as an OTA update (see step 4A), through an API device command (see step 4B), or you can upload the payload to the online IoTConnect platform and push an OTA update from there (see step 4C).
...
## 4B. Push Update as Command From Host Machine Console
...
## 4C. Upload/Push OTA Update in IoTConnect Online Platform
