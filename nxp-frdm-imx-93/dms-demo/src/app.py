...
#    3) Periodically reads /opt/demo/dms-data.json for new detection states (yawning,
...
#        - "set-thresholds": writes new threshold values to /opt/demo/dms-config.json
#          so the DMS script can pick them up (no Flask usage).
#        - "set-conditions": similarly writes forced states to /opt/demo/dms-config.json.
#
...
with open("/opt/demo/dms-processing.py", 'r') as file:
...
# DMS data read from /opt/demo/dms-data.json.
telemetry = {
...
# /opt/demo/dms-config.json. The DMS script will poll this file periodically
...
def update_config_file(transition_threshold=None, eye_threshold=None, forced_states=None):
    """
    Reads /opt/demo/dms-config.json, updates any fields that are provided,
...
DMS_process = subprocess.Popen(["python3", "/opt/demo/dms-processing.py"])
