...
Upgrades the /IOTCONNECT Starter Demo on the STM32MP257F-DK to the AWS Kinesis Video Streams (KVS) PutMedia video streaming demo.
...
This demo streams live video from a USB camera through the STM32MP257F-DK to AWS Kinesis Video Streams (KVS), accessible via the /IOTCONNECT platform. The KVS Producer SDK libraries are pre-built and bundled in the package — no on-device compilation is required.
...
```bash
cd /opt/demo
wget -O package.tar.gz https://raw.githubusercontent.com/avnet-iotconnect/iotc-python-lite-sdk-demos/main/stm32mp257f-dk/kvs-putmedia/package.tar.gz
...
2. Edit files in `stm32mp257f-dk/kvs-putmedia/src/` as needed.
