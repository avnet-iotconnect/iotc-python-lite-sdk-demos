...
This guide will help you create and deliver a package based on the /IOTCONNECT Starter Demo to the STM32MP257F-EV1.

> [!IMPORTANT]
> If you have not yet followed
> the [/IOTCONNECT quickstart guide for this board](https://github.com/avnet-iotconnect/iotc-python-lite-sdk-demos/blob/main/stm32mp257f-ev1/README.md),
...
Inside of the cloned repo (```iotc-python-lite-sdk-demos```), navigate to the ```stm32mp257f-ev1/starter-demo/src/```
directory:

```
cd ./stm32mp257f-ev1/starter-demo/src/
```
...
If the device will need to perform some actions (move files, install libaries, etc.) after the package is received,
modify ```install.sh``` to perform those actions. It will be automatically executed after the package is recevied and
...
> [!IMPORTANT]
> Notice that the password argument of the command is **the only arugment that is in quotes.** Make sure you pay
