...
Upgrades the /IOTCONNECT Starter Demo on the STM32MP257F-DK to the AWS Kinesis Video Streams (KVS) WebRTC live video streaming demo.
...
This demo streams live video from a USB camera through the STM32MP257F-DK to a browser or other WebRTC viewer via AWS Kinesis Video Streams (KVS) WebRTC. The device acts as a WebRTC MASTER: it connects to the KVS signaling channel provided by /IOTCONNECT and streams video to any VIEWER that connects.
...
1. Plug a USB camera into a USB port on the STM32MP257F-DK.
...
> [!IMPORTANT]
> This demo requires the `kvswebrtc` template (available [here](kvswebrtc-template.json)). The device **must be created in /IOTCONNECT with the `kvswebrtc` template** — the AWS backend provisions a KVS WebRTC signaling channel for `kvswebrtc` devices and a KVS stream for `kvsputmedia` devices, and these cannot be switched after device creation. If your device was created with `plitedemo` or `kvsputmedia`, create a new device using `kvswebrtc`.
...
```bash
cd /opt/demo
wget https://raw.githubusercontent.com/avnet-iotconnect/iotc-python-lite-sdk-demos/main/stm32mp257f-dk/kvs-webrtc/package.tar.gz
...
2. Edit files in `stm32mp257f-dk/kvs-webrtc/src/` as needed.

3. Rebuild the package:
   ```bash
   cd stm32mp257f-dk/kvs-webrtc
...
2. Create a new firmware if needed: click **Create Firmware** (top-right), name it, select the `kvswebrtc` template, set version numbers (e.g., `0`, `0`), browse to `package.tar.gz`, and click **Save**.
