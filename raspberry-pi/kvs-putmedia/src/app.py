...
# expose a hardware H264 *encoder* via V4L2 (only decode), so the USB camera's raw
# frames are re-encoded to H264 in software via x264enc. 640x480/30fps keeps the
# CPU encode load comfortable on both boards; the Pi 5's Cortex-A76 cores have
# ample headroom for higher resolutions if desired.
camera_options = {
    "video": {
        "width": 640,
        "height": 480,
...
def detect_video_device() -> Optional[str]:
    # The Raspberry Pi's own video hardware (bcm2835-codec / rp1-cfe) registers
    # several /dev/videoN nodes for hardware decode/ISP even with no CSI camera
    # attached, so picking the first device by number is not reliable. We check
    # for "usb" in the resolved sysfs path to reliably select the USB camera
    # regardless of which port it is plugged into or how many other video nodes
    # the board exposes.
    try:
